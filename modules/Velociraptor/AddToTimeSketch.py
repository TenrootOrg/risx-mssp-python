...
def start_timesketch(row, general_config, logger):
    ...
    try:
        ...
        subprocess.run(
            'sudo docker ps -a -q --filter "ancestor=log2timeline/plaso:20250918" --filter "status=exited" | xargs -r sudo docker rm -f',
...
"""
                        ...
                        command1 = f"sudo docker run --rm -v {host_extract_dir}:{host_extract_dir}:ro -v {plaso_host_dir}/:/data --cpus='{cpus}' --memory='{ram}' --user root log2timeline/plaso:20250918 log2timeline --parsers '{parser_arg}' --workers {num_workers} --temporary_directory /data --status_view window --status_view_interval 60 --logfile /data/log2timeline_{client_name}_{log_datetime}.log"
                        ...
                        pinfo_command = f"sudo docker run --rm -v {plaso_host_dir}/:/data -v {logs_dir}/:/logs log2timeline/plaso:20250918 pinfo -w /logs/pinfo_{client_name}_{log_datetime}.log /data/{client_name}Artifacts.plaso"
